...
MODEL="${1:-/tmp/qwen.gguf}"
if [[ ! -f "$MODEL" ]]; then
  cp "$ROOT/dashboard/models/Qwen2.5-0.5B-Instruct-Q4_K_M.gguf" /tmp/qwen.gguf
  MODEL=/tmp/qwen.gguf
fi
...
printf '%s\n' \
  ...
  '{"cmd":"quit"}' \
  | "$BIN" chat --jsonl --model "$MODEL" --log-level warn --max-context 256
